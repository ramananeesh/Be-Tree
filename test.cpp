...
#define DEFAULT_TEST_MAX_NODE_SIZE (4096)
#define DEFAULT_TEST_MIN_FLUSH_SIZE (DEFAULT_TEST_MAX_NODE_SIZE / 4)
// number of objects to retain in memory
#define DEFAULT_TEST_CACHE_SIZE (10)
#define DEFAULT_TEST_NDISTINCT_KEYS (10e+6)
#define DEFAULT_TEST_NOPS (10e+6)
...
void benchmark_upserts(betree<uint64_t, uint64_t> &b,
                       uint64_t nops,
                       uint64_t number_of_distinct_keys,
                       uint64_t random_seed)
{
  string input_file = "../../near-sortedness-code/source_code/input/data/workload/10-L/arr/createdata_1000000-elems_0-noise_10-lpercentage_1seed1588696639.dat";
  int *data;
  long int size = 0;
  ...
  for (int i = 0; i < n; i++)
  {
    // b.insert((uint64_t)data[i] + 1, to_string(data[i] + 1));
    b.insert((uint64_t)data[i] + 1, (uint64_t)data[i] + 1);
  }
  auto stop = std::chrono::high_resolution_clock::now();
  auto duration_insert = std::chrono::duration_cast<std::chrono::seconds>(stop - start).count();
  cout << "Time taken for insert = " << duration_insert << " seconds" << endl;
  ...
  auto duration_pquery = std::chrono::duration_cast<std::chrono::seconds>(stop - start).count();
  cout << "Time taken for point query of " << n << " elements = " << duration_pquery << " seconds" << endl;
